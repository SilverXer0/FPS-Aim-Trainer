...
public class TaskMLBridge_Headshot : MonoBehaviour
{
    public string taskName = "Headshot";
    [Range(0.5f,0.99f)] public float targetAcc = 0.85f; 
    void OnEnable()
    {
    ...
    void OnTaskEnd()
    {
        ...
        Debug.Log($"[Headshot ML] run#{runsSoFar+1} acc={acc:0.00} signed={meanSigned:0.0} abs={meanAbs:0.0}  {cur:0.0}→{next:0.0}");
    }
