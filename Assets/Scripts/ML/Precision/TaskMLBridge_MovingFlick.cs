...
public class TaskMLBridge_MovingFlick : MonoBehaviour
{
    [Tooltip("Unique key per scene, e.g., MovingFlick or MovingFlickRandom")]
    public string taskName = "MovingFlick";
    ...
    void OnTaskEnd()
    {
        ...
        Debug.Log($"[MovingFlick ML:{taskName}] run#{runs+1} acc={acc:0.00} signed={meanSigned:0.0} abs={meanAbs:0.0}  {cur:0.0}→{next:0.0}");
    }
